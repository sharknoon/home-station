...
<div class="card variant-filled shadow-2xl p-4 relative">
    ...
    <div>Published at: {dateTimeFormatter.format(new Date(app.publishedAt))}</div>
    <div>App repository: <a href={appRepositoryUrl} target="_blank" class="anchor">{appRepositoryUrl}</a></div>
</div>
