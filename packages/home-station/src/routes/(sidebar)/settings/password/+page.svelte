<script lang="ts">
    ...
    $: if (form?.success) {
        sendNotification('success', $i18n.t('settings.password.success'));
    } else if (form?.oldPassword) {
        sendNotification('error', $i18n.t('settings.password.wrong-old-password'));
    } else if (form?.error) {
...
<form method="post" action="?/updatePassword" class="space-y-4" use:enhance>
    ...
    >
        <Save class="mr-2" />{$i18n.t('settings.password.save')}
    </button>
</form>
